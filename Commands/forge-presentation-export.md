...
4. **Run the export.** Derive the output path by replacing the deck's `.md` extension with the
   ...
   node_modules/.bin/slidev export "<absolute-path-to-deck>" \
   --theme "$(pwd)/node_modules/@slidev/theme-default" \
   --format pdf --output "<absolute-path-to-deck-without-ext>.pdf"

   For PPTX:
   cd <workspace-root>/Plugins/presentation && \
   node_modules/.bin/slidev export "<absolute-path-to-deck>" \
   --theme "$(pwd)/node_modules/@slidev/theme-default" \
   --format pptx --output "<absolute-path-to-deck-without-ext>.pptx"
