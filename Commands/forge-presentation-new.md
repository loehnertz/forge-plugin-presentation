...
6. **Report.** Tell the user the output path, the slide count, and how to preview it:
   run `/forge-presentation-start`, or manually from `Plugins/presentation/`:
   `node_modules/.bin/slidev "<absolute-output-path>" --theme "$(pwd)/node_modules/@slidev/theme-default" --no-open`
